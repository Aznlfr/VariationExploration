...
\begin{figure}[H]
	...
	\caption{
		...
		are plotted against the corresponding quantities for the Uniform distribution $U(0,\beta_0)$
		 for different values of transmission rates $(\beta_0 = 2,4,6,8,10)$ in a simple SIR model ($m=1$).
		$\alpha$ and $\sigma$ are set to zero, and the duration of the simulation is $100$ time units, starting from the initial condition $y(0)=1^{-9}$.
		The number of steps is $3000.$
		As $\beta_0$ gets larger, $R_i$'s statistics get closer to those of the Uniform distribution.
	 }
\end{figure}
%\begin{figure}[H]
%	\centering
...
\end{equation}
\subsection{Using power means $\operatorname{PM}$ to simplify \Cref{eq_cv2g}}
By defining
\begin{align}
	\operatorname{PM}(l) = \frac{\int x^{l+1}f(x) dx}{\int x^{l}f(x) dx},
\end{align}
we have $\operatorname{PM}(1) = \mu_x(1 + \kappa_x)$ and
\begin{align*}
\operatorname{PM}(2) &= \frac{\int x^{3}f(x) dx}{\int x^2f(x) dx}\\
& = \frac{E(x^3)}{\mu^2_x(1 + \kappa_x)}.
\end{align*}
  \Cref{eq_cv2g} can be rewritten as follows:
\begin{align}
	\kappa_g = \frac{4}{3}\frac{\operatorname{PM}_i(2)}{\operatorname{PM}_i(1)} -1.
\end{align}
Moving $1$ to the left-hand side and replacing $1+\kappa_g$ by 
$\operatorname{PM}_g(1)/\operatorname{PM}_g(0)$
results in the equation:
\begin{align}
\frac{\operatorname{PM}_g(1)}{\operatorname{PM}_g(0)} = \frac{4}{3}\frac{\operatorname{PM}_i(2)}{\operatorname{PM}_i(1)}.
\end{align}
