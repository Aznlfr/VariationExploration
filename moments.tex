...
\begin{figure}[H]
	...
	\caption{Variance in $R_c$ for an SIR model with nonlinear transmission (\Cref{eq_Erlang} with $m=1$, $\sigma=0$, $\kappa=0, 1, 2, 3$, and $\omega=0$).
...
\begin{align} \label{eq_phi_original}
...
For $x \sim \operatorname{U}(a,b)$, the mean, variance, and the third raw moments are equal to
\begin{align*}
	\mu & =\frac{1}{2}(a+b) \\
	\sigma^2 & = \frac{1}{12}(a-b)^2\\
	\operatorname{E}(t^3) & =\underbrace{\frac{1}{4}(a+b)}_{\frac{\mu}{2}}\underbrace{(a^2 + b^2)}_{12\sigma^2 + 2ab}.
\end{align*}
Substituting  $\operatorname{E}(t^3)$ into  \Cref{eq_phi}, results in 
\begin{align}
\frac{\mu}{2}(12\sigma^2 + 2ab) &= \mu^3 (1+\kappa)^2(1+\phi)\\
\frac{1}{2}(12\sigma^2 + 2ab) &= \mu^2 (1+\kappa)^2(1+\phi)\\
\frac{(6\sigma^2 + ab)}{\mu^2} &= (1+\kappa)^2(1+\phi)\\
6\kappa + \frac{ab}{\mu^2} &= (1+\kappa)^2(1+\phi)\\
\frac{6\kappa + \frac{ab}{\mu^2}}{(1+\kappa)^2}-1 &= \phi \label{eq_phi_unif}
\end{align}
Given  $\kappa$ equals $\sigma^2/\mu^2$, we have
\begin{align*}
\kappa &= \frac{1}{3}\frac{(a-b)^2}{(a+b)^2}\\
3\kappa & = \frac{(a-b)^2}{(a+b)^2}\\
3\kappa -1  & = \frac{(a-b)^2 - (a+b)^2}{(a+b)^2}\\
3\kappa -1  & = \frac{-4ab}{(a+b)^2}\\
3\kappa -1  & =  \frac{-ab}{\mu^2}
\end{align*}
Substituting $-(3\kappa -1)$ for $\frac{ab}{\mu^2}$ in the LHS of \Cref{eq_phi_unif} results in 
\begin{align}
 \phi = \frac{3\kappa + 1}{(1+\kappa)^2}-1. 
\end{align}
