...
UniformSecRaw <- (u_b_Uniform^2 + l_b_Uniform^2 + l_b_Uniform*u_b_Uniform)/3
...
UniformStat<-data.frame(B0 = B0vec, mu = UniformMu,
                        secondRaw = UniformSecRaw,
                        thirdRaw = UniformThirdRaw)
firstRawInfectious <-1
secondRawInfectious <-2
thirdRawInfectious <- 6
res_mat$UniformMu <- firstRawInfectious*UniformStat[match(res_mat$B0,
                                                          UniformStat$B0),"mu"]
res_mat$UniformSecRaw <- secondRawInfectious*UniformStat[match(res_mat$B0,
                                                               UniformStat$B0),
                                                         "secondRaw"]
res_mat$Uniform3Raw <- thirdRawInfectious* UniformStat[match(res_mat$B0,
                                                             UniformStat$B0),
                                                       "thirdRaw"]
